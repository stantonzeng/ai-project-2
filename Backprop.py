...
def finding_neighbor(features_list, myset):
    ...
    for i in range(1, len(features_list)): #For each feature
        correct = 0
        if(myset[i] != 1): #As long as we have not checked this feature yet
            continue 
        for x in range(len(features_list[0])): #Which object we are measuring (out of 500)
            ...
            for f in range(len(features_list[0])): #Object that we are comparing it with
                ...
                # print(get_sum(features_list, myset, x, f, i) )
                # print(((features_list[i][x]-features_list[i][f])**2))
                # print(i, x, f)
                d = math.sqrt( get_sum(features_list, myset, x, f, i) - ((features_list[i][x]-features_list[i][f])**2) )#figure out why this is wrong!!!
                # print("d:", d, "using features", i)
                if d < distance_apart:
                    nearest_neighbor = f
                    nearest_class = features_list[0][f]
                    distance_apart = d

            # print(x+1, "is nearest neighbor with", nearest_neighbor+1, "at class", nearest_class, "discarding feature", i)
    ...
    print("The best accuracy is:", best_accuracy, "removing feature", best_feature, "for the set" ,temp_set)
    


if __name__ == "__main__":
    things = open_small(54)
    size = 10
